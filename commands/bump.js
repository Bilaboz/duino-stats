...
module.exports.run = async (client, message, args) => {
    ...
    const filter = m => m.embeds[0] && m.embeds[0].description.includes("done");
    const result = await message.channel.awaitMessages(filter, { max: 1, time: 15000, errors: ["time"] })
    ...
    Profile.findOne({
        ...
        if (!query) {
            ...
            message.channel.send(`Bump added! **${message.author.username}** now has 1 bump!`);
        } else {
            query.bumps += 1;
