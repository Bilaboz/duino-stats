...
module.exports.run = async (client, message, args, color) => {
    const ducoAmount = parseFloat(args[0]);
    if (!ducoAmount || isNaN(ducoAmount)) return message.channel.send("Please specify a valid number of DUCO to deposit");
    ...
    const coinAmount = Math.round(ducoAmount * 100);
    ...
    if (!ducoBalance || ducoBalance < ducoAmount) {
        embed.setColor(color.red);
        embed.setDescription(`\`ERROR\`: The specified username isn't listed in the API or doesn't have enough balance to exchange ${ducoAmount} <:duco:807188450393980958>`);
        return msg.edit({ embeds: [embed] });
    }
    ...
    const msgCollected = await message.channel.awaitMessages({ filter: filterMessage, time: 30000, max: 1 });
